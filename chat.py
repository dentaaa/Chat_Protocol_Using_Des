...
class Chat:
    def __init__(self):
        self.sessions = {}
        self.users = {}
        self.users['denta'] = {'nama': 'denta', 'negara': 'Indonesia',
                               'password': '1234', 'incoming': {}, 'outgoing': {}}
        self.users['clement'] = {'nama': 'clement', 'negara': 'Jerman',
                                 'password': '1234', 'incoming': {}, 'outgoing': {}}
        self.users['anya'] = {'nama': 'Anya Geraldine', 'negara': 'Indonesia',
                              'password': '1234', 'incoming': {}, 'outgoing': {}}
    ...
    def get_inbox(self, username):
        ...
        # try_decrypt = s_fr['incoming']['msg']
        # decrypted = des.decrypt(key, try_decrypt, padding=True)
        # s_fr['incoming']['msg'] = decrypted

        incoming = s_fr['incoming']
        msgs = {}
        for users in incoming:
            msgs[users] = []
            # try_decrypt = msgs[users][0]['msg']
            # decrypted = des.decrypt(key, try_decrypt, padding=True)
            # msgs[users][0]['msg'] = decrypted
            while not incoming[users].empty():
                msgs[users].append(s_fr['incoming'][users].get_nowait())
                try_decrypt = msgs[users][0]['msg']
                try_decrypt = try_decrypt[1:]
                try_decrypt = try_decrypt[:-1:]
                try_decrypt = try_decrypt[:-1:]
                try_decrypt = try_decrypt[:-1:]
                # try_decrypt = try_decrypt[:-1:]
                print(try_decrypt)
                decrypted = self.decrypt(key, try_decrypt, padding=True)
                msgs[users][0]['msg'] = decrypted
